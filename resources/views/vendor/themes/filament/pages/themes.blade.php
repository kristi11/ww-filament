@php use Hasnayeen\Themes\Contracts\HasChangeableColor; @endphp
@php use Hasnayeen\Themes\Contracts\HasOnlyDarkMode; @endphp
@php use Hasnayeen\Themes\Contracts\HasOnlyLightMode; @endphp
        ...
        <div class="flex items-center gap-4 border-t py-6">
            @if ($this->getCurrentTheme() instanceof HasChangeableColor)
                ...
                <div class="flex items-center space-x-4 rtl:space-x-reverse">
                    <input type="color" id="custom" name="custom" class="w-4 h-4"
                           wire:change="setColor($event.target.value)" value=""/>
    ...
    <section class="">
        ...
        <div class="grid grid-cols-1 gap-6 border-t py-6">
            @foreach ($this->getThemes() as $name => $theme)
                @php
                    $noLightMode = in_array(HasOnlyDarkMode::class, class_implements($theme));
                    $noDarkMode = in_array(HasOnlyLightMode::class, class_implements($theme));
                    $supportColorChange = in_array(HasChangeableColor::class, class_implements($theme));
                @endphp
                        ...
                        <div class="flex items-center space-x-4">
                            ...
                            @if ($supportColorChange)
                                <span
                                    ...
                                    <svg class="w-4 h-4 dark:text-gray-800" viewBox="0 0 24 24" fill="none"
                                         stroke="currentColor"
